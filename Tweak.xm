...
- (void)normalizePagedLayoutForOverlay:(UIViewController *)overlay {
    ...
    // TEMPORARY DIAGNOSTIC (remove once confirmed): unconditionally paint a
    // bright red border, regardless of what CCAIsLandscapeOrientation
    // returns. This checks a more basic question first — does this
    // function even run at all when CC is opened over a landscape-locked
    // app? If no border shows up, the whole hook is being bypassed in this
    // scenario and the orientation-detection logic isn't the problem.
    overlay.view.layer.borderColor = [UIColor redColor].CGColor;
    overlay.view.layer.borderWidth = 6.0;
