...
static void CCARestoreConnectivityContainerMaterial(UIView *root) {
    ...
    while (container && ![NSStringFromClass(container.class) isEqualToString:@"CCUIContentModuleContentContainerView"]) container = container.superview;
    if (!container) return;
    // The suppress functions (CCASuppressConnectivityOuterMaterial,
    // CCASuppressConnectivityStockContainerMaterial) hide material/backdrop
    // views at ANY depth via a recursive walk. Restoring only the direct
    // children here means a material suppressed a level or two deeper never
    // comes back — the compact card's backdrop is left permanently blank
    // until the separate 0.8s watchdog cleanup happens to catch it. Walk
    // the full subtree the same way the suppress side does.
    NSMutableArray<UIView *> *queue = [NSMutableArray arrayWithArray:container.subviews];
    while (queue.count) {
...
- (void)contentModuleContainerViewController:(id)container willCloseExpandedModule:(id)module {
    ...
    // Sibling modules (flashlight, personal hotspot, etc.) become visible
    // again the instant this flag flips, but the connectivity card's own
    // expanded rows are hidden by a separate call to
    // CCAConfigureConnectivityLayout from a different delegate path. When
    // those two don't land on the same runloop turn, there's a window where
    // the still-visible expanded connectivity rows overlap the
    // now-visible-again sibling modules underneath — the double-exposure
    // flash. Force the connectivity collapse to happen right here, at the
    // same moment, so both sides of the transition move together.
    UIViewController *connectivityForClose = CCAConnectivityChild(overlay, @"CCUIConnectivityModuleViewController");
    if (connectivityForClose) CCAConfigureConnectivityLayout(connectivityForClose);
    if (overlay && !gCCAControlCenterPresented) {
        [[CCAsterCoordinator shared] animateOwnedDuplicateHostForOverlay:overlay presented:NO];
    } else if (overlay) {
