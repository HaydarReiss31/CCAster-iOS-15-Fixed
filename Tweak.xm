...
- (void)normalizePagedLayoutForOverlay:(UIViewController *)overlay {
    ...
    // TEMPORARY DIAGNOSTIC (remove once landscape detection is confirmed
    // working): paint a bright red border around the whole overlay whenever
    // landscape is detected, so a screenshot can confirm whether detection
    // is firing at all, instead of guessing blind again.
    overlay.view.layer.borderColor = CCAIsLandscapeOrientation(overlay.view) ? [UIColor redColor].CGColor : [UIColor clearColor].CGColor;
    overlay.view.layer.borderWidth = CCAIsLandscapeOrientation(overlay.view) ? 6.0 : 0.0;
