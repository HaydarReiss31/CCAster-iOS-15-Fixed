...
static BOOL CCAIsLandscapeOrientation(UIView *view) {
    ...
    // NOT actually resized to landscape bounds — the whole window is kept
    // portrait-shaped and rotated with a transform instead. Checking
    // bounds.size here would always see portrait dimensions and never
    // detect landscape at all, so check the rotation that's actually
    // applied (via the window transform, falling back to the status bar
    // orientation) rather than the view's raw size.
    UIWindow *window = view.window;
    if (window) {
        CGAffineTransform t = window.transform;
        // A 90/270 degree rotation has b and c populated (~ +/-1) and a/d
        // near 0; an identity or 180-degree transform does not.
